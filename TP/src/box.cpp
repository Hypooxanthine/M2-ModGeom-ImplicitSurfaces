...
\code
Box box(glm::vec3(0.0,0.0,0.0),glm::vec3(1.0,1.0,1.0)); // Unit box
glm::vec3 a=box[0]; // Lower vertex
glm::vec3 b=box[1]; // Opposite vertex
\endcode
*/

#include <glm/gtx/string_cast.hpp>

const float Box::epsilon = 1.0e-5f; //!< Epsilon value used to check intersections and some round off errors.
...
*/
Box::Box(const glm::vec3& c, float r)
{
  a = c - glm::vec3(r);
  b = c + glm::vec3(r);
}
...
\code
Box box(glm::vec3::Min(a,b),glm::vec3::Max(a,b));
\endcode
...
\code
Box box(glm::vec3(0.0),2.0);  // Simplified constructor Box(2.0);
\endcode
\param r Half side length.
*/
Box::Box(float r)
...
{
  s << "Box(" << glm::to_string(box.a) << ',' << glm::to_string(box.b) << ")";
  return s;
}
