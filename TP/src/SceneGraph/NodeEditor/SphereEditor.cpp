...
void SphereEditor::onImgui_Impl()
{
    ...
    ImGui::SameLine();
    ImGui::SliderFloat3("##Spherecenter", reinterpret_cast<float*>(&m_Implicit->getCenterRef()), -10.f, 10.f, "%.2f", ImGuiSliderFlags_Logarithmic);

    ImGui::TextWrapped("Sphere radius");
    ImGui::SameLine();
    ImGui::SliderFloat("##Sphereradius", &m_Implicit->getRadiusRef(), 0.f, 10.f, "%.2f", ImGuiSliderFlags_Logarithmic);
}